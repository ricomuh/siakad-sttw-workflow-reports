...
const { features, accounts } = require('./p3m-workflow-config.cjs');
...
function archiveExistingReport(reportPath, today) {
  ...
  const matchedDate = current.match(/\*\*Tanggal\*\*:\s*(\d{4}-\d{2}-\d{2})/)
    || current.match(/\*Generated:\s*(\d{4}-\d{2}-\d{2})\*/);
...
function buildReport(feature, results, today) {
  ...
  markdown += '- Data yang tampil mengikuti seeder P3M yang aktif saat scan dijalankan.\n';
...
function main() {
  ...
  console.log(`\nGenerated ${generatedCount} P3M reports`);
}
