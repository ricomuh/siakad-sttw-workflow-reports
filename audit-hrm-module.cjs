...
const { accounts, features } = require('./hrm-workflow-config.cjs');
...
async function executeOpen(page, step, openSpec, accountKey) {
  ...
  if (openSpec.mode === 'click') {
    ...
    const response = await clickMainText(page, openSpec.text);
...
async function analyzeStep(page, response, step, openMeta, screenshot) {
  ...
  const flags = getBodyTextFlags(bodyText);
  const statusCode = response?.status() ?? null;
  const url = page.url();

  if (url.includes('/login') || flags.isLogin || statusCode === 401) {
    return {
      ok: false,
      url,
      statusCode,
  ...
  if (statusCode === 500 || flags.isServerError) {
    return {
      ok: false,
      url,
      statusCode,
  ...
  if (statusCode === 403 || flags.isPermissionError) {
    return {
      ok: false,
      url,
      statusCode,
  ...
  if (statusCode === 404 || statusCode === 405 || flags.isClientError) {
    return {
      ok: false,
      url,
      statusCode,
  ...
  if (!isFormStep && flags.incompleteData) {
    issues.push({
  ...
  if (!isFormStep && flags.missingFeature) {
    issues.push({
